...
def download_athena_script_file(file_name):

    local_file_name = file_name.replace(":", "_")
    # If file already exists locally do not re-download game
    if os.path.isfile(f"{local_file_name}"):
        return
    directory = "scripts/"
    with requests.get(f"{S3_BUCKET_URL}/athena-ready/ddl/{file_name}", stream=True) as r:
        r.raise_for_status()
        with open(f"{directory}{local_file_name}", "wb") as f:
            print(f"Writing {file_name} to disk")
            for chunk in r.iter_content(chunk_size=8192):
                f.write(chunk)


def download_athena_scripts():
    directory = "scripts"
    if not os.path.exists(directory):
        os.makedirs(directory)
    files = ["games.sql", "leagues.sql", "mapping-data.sql", "players.sql", "teams.sql", "tournaments.sql"]
    for file in files:
        download_athena_script_file(file)


if __name__ == "__main__":
#    download_esports_files()
#    download_games(2023)
    download_athena_scripts()
